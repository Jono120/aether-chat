...
export function applyFuzzingStrategyToLabel(label, strategy = 'grid_snap') {
...
export function applyFuzzingStrategyToProfile(profile, strategy) {
  if (!profile?.fuzzedDistance) return profile;
  return {
    ...profile,
    fuzzedDistance: applyFuzzingStrategyToLabel(profile.fuzzedDistance, strategy),
  };
}

export function applyFuzzingStrategyToProfiles(profiles, strategy) {
  return profiles.map((p) => applyFuzzingStrategyToProfile(p, strategy));
}
